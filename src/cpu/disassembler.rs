...
const EXTENDED_INSTRUCTIONS: [Instruction; 256] = [
    (0x00, "RLC  B", 2),
    (0x01, "RLC  C", 2),
    (0x02, "RLC  D", 2),
    (0x03, "RLC  E", 2),
...
pub fn decode_instruction(opcode: u8) -> &'static str {
...
pub fn disassemble_rom(rom: &[u8]) -> String {
    ...
    while addr < u16::MAX - 1 {
        ...
        let instr_opt = if *opcode == 0xcb {
            let extended_opcode = rom[(addr + 1) as usize];
            EXTENDED_INSTRUCTIONS.get(extended_opcode as usize)
        } else {
            INSTRUCTIONS.get(*opcode as usize)
        };

        let Some((_, mnemonic, length)) = instr_opt else {
            return ret;
        };
